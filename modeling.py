...
class BertForSequenceClassificationDualLossNewSimilarity(BertPreTrainedModel):
    ...
    The cosine similarity is computed between ii and iii outputs
    The cosine similarity will be concatenated with output from i and inserted to the dense layer as in the original architecture.

    '''
    def __init__(self, config, num_labels):
        super(BertForSequenceClassificationDualLossNewSimilarity, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size + 1, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, sim_labels=None):
        ...
        cos_sim = self.cosine(pooled_output_sen1, pooled_output_sen2).unsqueeze(1)
